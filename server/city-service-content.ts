// City-Service Content Hub - Unique content for top market city-service combinations
// This replaces thin templated content with rich, city-specific SEO content

export interface CityServiceContent {
  slug: string;
  cityName: string;
  stateName: string;
  serviceName: string;
  h1: string;
  metaTitle: string;
  metaDescription: string;
  heroDescription: string;
  sections: Array<{
    heading: string;
    content: string;
  }>;
  localVenues: string[];
  localEvents: string[];
  faqs: Array<{
    question: string;
    answer: string;
  }>;
  pricingNote: string;
  areasServed: string[];
}

// Import city-specific content
import { losAngelesContent } from './city-content/los-angeles';
import { newYorkCityContent } from './city-content/new-york-city';
import { miamiContent } from './city-content/miami';
import { chicagoContent } from './city-content/chicago';
import { lasVegasContent } from './city-content/las-vegas';
import { denverContent } from './city-content/denver';
import { houstonContent } from './city-content/houston';
import { dallasContent } from './city-content/dallas';
import { phoenixContent } from './city-content/phoenix';
import { atlantaContent } from './city-content/atlanta';
import { seattleContent } from './city-content/seattle';
import { nashvilleContent } from './city-content/nashville';
import { bostonContent } from './city-content/boston';
import { sanFranciscoContent } from './city-content/san-francisco';
import { philadelphiaContent } from './city-content/philadelphia';
import { sanDiegoContent } from './city-content/san-diego';
import { portlandContent } from './city-content/portland';
import { detroitContent } from './city-content/detroit';
import { minneapolisContent } from './city-content/minneapolis';
import { charlotteContent } from './city-content/charlotte';
import { tampaContent } from './city-content/tampa';
import { baltimoreContent } from './city-content/baltimore';
import { stLouisContent } from './city-content/st-louis';
import { cincinnatiContent } from './city-content/cincinnati';
import { pittsburghContent } from './city-content/pittsburgh';
import { indianapolisContent } from './city-content/indianapolis';
import { newOrleansContent } from './city-content/new-orleans';
import { kansasCityContent } from './city-content/kansas-city';
import { washingtonDcContent } from './city-content/washington-dc';
import { sanAntonioContent } from './city-content/san-antonio';
import { columbusContent } from './city-content/columbus';
import { raleighContent } from './city-content/raleigh';
import { sacramentoContent } from './city-content/sacramento';
import { saltLakeCityContent } from './city-content/salt-lake-city';
import { orlandoContent } from './city-content/orlando';
import { jacksonvilleContent } from './city-content/jacksonville';
import { oklahomaCityContent } from './city-content/oklahoma-city';
import { memphisContent } from './city-content/memphis';
import { louisvilleContent } from './city-content/louisville';
import { richmondContent } from './city-content/richmond';
import { albuquerqueContent } from './city-content/albuquerque';
import { tucsonContent } from './city-content/tucson';
import { elPasoContent } from './city-content/el-paso';
import { batonRougeContent } from './city-content/baton-rouge';
import { fresnoContent } from './city-content/fresno';
import { bakersfieldContent } from './city-content/bakersfield';
import { renoContent } from './city-content/reno';
import { coloradoSpringsContent } from './city-content/colorado-springs';
import { stocktonContent } from './city-content/stockton';
import { spokaneContent } from './city-content/spokane';
import { sanJoseContent } from './city-content/san-jose';
import { arlingtonContent } from './city-content/arlington';
import { fortWorthContent } from './city-content/fort-worth';
import { corpusChristiContent } from './city-content/corpus-christi';
import { laredoContent } from './city-content/laredo';
import { anchorageContent } from './city-content/anchorage';
import { boiseContent } from './city-content/boise';
import { milwaukeeContent } from './city-content/milwaukee';
import { austinContent } from './city-content/austin';
import { clevelandContent } from './city-content/cleveland';

// Combine all city content into a single lookup
const allCityServiceContent: Record<string, CityServiceContent> = {
  ...losAngelesContent,
  ...newYorkCityContent,
  ...miamiContent,
  ...chicagoContent,
  ...lasVegasContent,
  ...denverContent,
  ...houstonContent,
  ...dallasContent,
  ...phoenixContent,
  ...atlantaContent,
  ...seattleContent,
  ...nashvilleContent,
  ...bostonContent,
  ...sanFranciscoContent,
  ...philadelphiaContent,
  ...sanDiegoContent,
  ...portlandContent,
  ...detroitContent,
  ...minneapolisContent,
  ...charlotteContent,
  ...tampaContent,
  ...baltimoreContent,
  ...stLouisContent,
  ...cincinnatiContent,
  ...pittsburghContent,
  ...indianapolisContent,
  ...newOrleansContent,
  ...kansasCityContent,
  ...washingtonDcContent,
  ...sanAntonioContent,
  ...columbusContent,
  ...raleighContent,
  ...sacramentoContent,
  ...saltLakeCityContent,
  ...orlandoContent,
  ...jacksonvilleContent,
  ...oklahomaCityContent,
  ...memphisContent,
  ...louisvilleContent,
  ...richmondContent,
  ...albuquerqueContent,
  ...tucsonContent,
  ...elPasoContent,
  ...batonRougeContent,
  ...fresnoContent,
  ...bakersfieldContent,
  ...renoContent,
  ...coloradoSpringsContent,
  ...stocktonContent,
  ...spokaneContent,
  ...sanJoseContent,
  ...arlingtonContent,
  ...fortWorthContent,
  ...corpusChristiContent,
  ...laredoContent,
  ...anchorageContent,
  ...boiseContent,
  ...milwaukeeContent,
  ...austinContent,
  ...clevelandContent,
};

export function getCityServiceContent(slug: string): CityServiceContent | undefined {
  return allCityServiceContent[slug];
}

export function hasEnrichedContent(slug: string): boolean {
  return slug in allCityServiceContent;
}
